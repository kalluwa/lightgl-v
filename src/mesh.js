...
Mesh.prototype = {
  ...
  transform: function(matrix) {
    this.vertices = this.vertices.map(function(v) {
      return matrix.transformPoint(Vector3.fromArray(v)).toArray();
    });
    if (this.normals) {
      var invTrans = matrix.inverse().transpose();
      this.normals = this.normals.map(function(n) {
        return invTrans.transformVector(Vector3.fromArray(n)).unit().toArray();
      });
    }
    this.compile();
    return this;
  ...
  computeNormals: function() {
    ...
    for (var i = 0; i < this.triangles.length; i++) {
      ...
      var a = Vector3.fromArray(this.vertices[t[0]]);
      var b = Vector3.fromArray(this.vertices[t[1]]);
      var c = Vector3.fromArray(this.vertices[t[2]]);
  ...
  getAABB: function() {
    var aabb = { min: new Vector3(Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE) };
    aabb.max = aabb.min.negative();
    for (var i = 0; i < this.vertices.length; i++) {
      var v = Vector3.fromArray(this.vertices[i]);
      aabb.min = Vector3.min(aabb.min, v);
      aabb.max = Vector3.max(aabb.max, v);
    }
    return aabb;
  },
  ...
  getBoundingSphere: function() {
    ...
    for (var i = 0; i < this.vertices.length; i++) {
      sphere.radius = Math.max(sphere.radius,
        Vector3.fromArray(this.vertices[i]).subtract(sphere.center).length());
    }
    return sphere;
  }
};
...
function pickOctant(i) {
  return new Vector3((i & 1) * 2 - 1, (i & 2) - 1, (i & 4) / 2 - 1);
}
  ...
  for (var octant = 0; octant < 8; octant++) {
    ...
    for (var i = 0; i <= detail; i++) {
      ...
      for (var j = 0; i + j <= detail; j++) {
        ...
        var vertex = { vertex: new Vector3(fix(a), fix(b), fix(c)).unit().multiply(scale).toArray() };
