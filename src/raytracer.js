...
//       tracer.eye, ray, new GL.Vector3(0, 0, 0), 1);
function Raytracer() {
  var v = gl.getParameter(gl.VIEWPORT);
  var m = gl.modelviewMatrix.m;

  var axisX = new Vector3(m[0], m[4], m[8]);
  var axisY = new Vector3(m[1], m[5], m[9]);
  var axisZ = new Vector3(m[2], m[6], m[10]);
  var offset = new Vector3(m[3], m[7], m[11]);
  this.eye = new Vector3(-offset.dot(axisX), -offset.dot(axisY), -offset.dot(axisZ));
...
Raytracer.prototype = {
  ...
  getRayForPixel: function(x, y) {
    ...
    var ray0 = Vector3.lerp(this.ray00, this.ray10, x);
    var ray1 = Vector3.lerp(this.ray01, this.ray11, x);
    return Vector3.lerp(ray0, ray1, y).unit();
  }
};
...
Raytracer.hitTestBox = function(origin, ray, min, max) {
  ...
  if (tNear > 0 && tNear < tFar) {
    ...
    return new HitTest(tNear, hit, new Vector3(
