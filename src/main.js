...
var GL = {
  ...
  create: function(options) {
    ...
    try { gl = canvas.getContext('webgl2', options); console.log("webgl2");} catch (e) { alert("webgl2 not supported! try webgl"); }
    if (!gl) try { gl = gl || canvas.getContext('webgl', options) || canvas.getContext('experimental-webgl', options); console.log("webgl");} catch (e) { alert("webgl not supported") }
...
function addMatrixStack() {
  ...
  gl.project = function(objX, objY, objZ, modelview, projection, viewport) {
    ...
    var point = projection.transformPoint(modelview.transformPoint(new Vector3(objX, objY, objZ)));
    return new Vector3(
